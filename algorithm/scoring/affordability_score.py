import pandas as pd
from config import AFFORDABILITY_WEIGHTS

def calculate_affordability_score(features: pd.DataFrame) -> pd.Series:
    weighted_score = pd.Series(0.0, index=features.index)
    available_weight = pd.Series(0.0, index=features.index)

    for feature, weight in AFFORDABILITY_WEIGHTS.items():
